	...
	final class Alg_WC_Email_Verification {
		...
		function move_wc_settings_tab_to_wpfactory_menu() {
			...
			$wpf_admin_menu->move_wc_settings_tab_to_wpfactory_menu( array(
				...
				'page_title'         => __( 'Customer Email Verification for WooCommerce', 'emails-verification-for-woocommerce' ),
				'plugin_icon'        => array(
					'get_url_method'    => 'wporg_plugins_api',
					'wporg_plugin_slug' => 'emails-verification-for-woocommerce',
...
$alg_wc_ev_activation_hook = 'alg_wc_ev_on_activation';
register_activation_hook( __FILE__, function () use ( $alg_wc_ev_activation_hook ) {
	add_option( $alg_wc_ev_activation_hook, 'yes' );
} );
register_deactivation_hook( __FILE__, function () {
	do_action( 'alg_wc_ev_on_deactivation' );
} );
add_action( 'admin_init', function () use ( $alg_wc_ev_activation_hook ) {
	if ( is_admin() && get_option( $alg_wc_ev_activation_hook ) === 'yes' ) {
		delete_option( $alg_wc_ev_activation_hook );
		do_action( 'alg_wc_ev_on_activation' );
	}
} );