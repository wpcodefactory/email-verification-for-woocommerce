	...
	class Alg_WC_Email_Verification_Admin {
		...
		function filter_users_based_on_verification_status( WP_User_Query $query ) {
			...
			if ( is_admin() && 'users.php' === $pagenow ) {
				$user_status_top              = sanitize_text_field( filter_input( INPUT_GET, 'alg_wc_ev_verification_status_top' ) );
				$user_status_bottom           = sanitize_text_field( filter_input( INPUT_GET, 'alg_wc_ev_verification_status_bottom' ) );
		...
		function add_user_verification_status_filter_options( $which ) {
			...
			$user_status = sanitize_text_field( filter_input( INPUT_GET, 'alg_wc_ev_verification_status_' . $which ) );
			$options     = array(
			...
			printf( '<select name="alg_wc_ev_verification_status_%s" style="float:none;margin-left:10px; margin-right: 6px;">%s</select>', esc_attr( $which ), wp_kses( implode( '', $options ), array( 'option' => array( 'value' => array(), 'selected' => array() ) ) ) );
		...
		function manage_bulk_notices() {
			if (
				'yes' === sanitize_text_field( filter_input( INPUT_GET, 'bkg_process' ) )
			) {
				$complete_bkg_task_msg_regarding_email = alg_wc_ev_get_complete_bkg_task_msg_regarding_email();
			}

			if ( ( $count = filter_input( INPUT_GET, 'bulk_alg_wc_ev_resend', FILTER_VALIDATE_INT ) ) ) {
				/* translators: %d: number of users */
				printf( '<div id="message" class="updated notice is-dismissable"><p>' . esc_html( _n( 'Verification email sent to %d user successfully.', 'Verification email sent to %d users successfully.', $count, 'emails-verification-for-woocommerce' ) ) . '</p></div>', (int) $count );
			}

			// For verify user action
			if ( ( $count = filter_input( INPUT_GET, 'bulk_alg_wc_ev_verify_users', FILTER_VALIDATE_INT ) ) ) {
				if ( 'yes' === sanitize_text_field( filter_input( INPUT_GET, 'bkg_process' ) ) ) {
					/* translators: %d: number of users */
					...
					printf( '<div id="message" class="updated notice is-dismissable"><p>' . esc_html( $bkg_task_msg ) . '</p></div>' );
				} else {
					/* translators: %d: number of users */
					printf( '<div id="message" class="updated notice is-dismissable"><p>' . esc_html( _n( '%d user verified successfully.', '%d users verified successfully.', $count, 'emails-verification-for-woocommerce' ) ) . '</p></div>', (int) $count );
				}
			}

			// For unverify user action
			if ( ( $count = filter_input( INPUT_GET, 'bulk_alg_wc_ev_unverify_users', FILTER_VALIDATE_INT ) ) ) {
				if ( 'yes' === sanitize_text_field( filter_input( INPUT_GET, 'bkg_process' ) ) ) {
					/* translators: %d: number of users */
					...
					/* translators: %d: number of users */
					printf( '<div id="message" class="updated notice is-dismissable"><p>' . esc_html( _n( '%d user unverified successfully.', '%d users unverified successfully.', $count, 'emails-verification-for-woocommerce' ) ) . '</p></div>', (int) $count );
				}
			}
		}
		...
		$nonce_value = isset( $_GET['_alg_wc_ev_wpnonce'] ) ? sanitize_text_field( wp_unslash( $_GET['_alg_wc_ev_wpnonce'] ) ) : '';
			if ( $do_check_nonce && isset( $_GET[ $action ] ) && ( '' === $nonce_value || ! wp_verify_nonce( $nonce_value, 'alg_wc_ev_action' ) ) ) {
				wp_die( esc_html__( 'Nonce not found or not verified.', 'emails-verification-for-woocommerce' ) );
			}
			$action_value = isset( $_GET[ $action ] ) ? sanitize_text_field( wp_unslash( $_GET[ $action ] ) ) : '';
			if ( ! empty( $action_value ) && ( $user_id = intval( $action_value ) ) > 0 && $this->is_user_allowed_to_interact_with_admin_ui() ) {
				return $user_id;
			}
		...
		function admin_manual_actions() {
			...
			if ( $user_id = $this->get_user_id_from_action( 'alg_wc_ev_admin_verify_done', false ) ) {
				/* translators: %s: user info with ID */
		...
		function admin_notices() {
			if ( isset( $this->admin_notice ) ) {
				echo '<div class="notice notice-success is-dismissible"><p>' . wp_kses_post( $this->admin_notice ) . '</p></div>';
			}
		}
				...
				if ( $user && ! is_wp_error( $user ) ) {
					...
					if ( ! alg_wc_ev()->core->is_user_verified( $user ) ) {
						...
						if ( $activation_code_time ) {
							$activation_code_time = ' (' . gmdate( 'Y-m-d H:i:s', $activation_code_time ) . ')';
						}
		...
		function generate_editor_html( $value ) {
			...
			<tr class="<?php echo esc_attr( $value['row_class'] ); ?>">
				...
				<td class="forminp forminp-<?php echo esc_attr( sanitize_title( $value['type'] ) ); ?> alg-wc-editor">
					...
					<div<?php foreach ( $custom_attributes as $attr => $val ) {
		...
		function enqueue_ev_setting_style_and_script() {
			if (
				'wc-settings' === sanitize_text_field( filter_input( INPUT_GET, 'page' ) ) && 'alg_wc_ev' === sanitize_text_field( filter_input( INPUT_GET, 'tab' ) ) && in_array( sanitize_text_field( filter_input( INPUT_GET, 'section' ) ), array( 'emails', 'messages' ), true )
			) {
				?>
				<style>
