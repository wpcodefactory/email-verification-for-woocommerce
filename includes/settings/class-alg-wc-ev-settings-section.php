...
if ( ! class_exists( 'Alg_WC_Email_Verification_Settings_Section' ) ) :
	...
	class Alg_WC_Email_Verification_Settings_Section {
		...
		function get_block_unverify_login_option_warning( $args = null ) {
			...
			/* translators: %1$s: option name, %2$s: option status */

			return sprintf( __( 'This will probably make more sense with the %1$s option %2$s, where the account verification becomes optional.', 'emails-verification-for-woocommerce' ), '<strong>' . __( 'Block unverified login', 'emails-verification-for-woocommerce' ) . '</strong>', '<strong>' . $option_status . '</strong>' );
		}
		...
		function available_placeholders_desc( $placeholders ) {
			/* translators: %s: comma-separated list of placeholder codes */
		...
		function separate_email_option_msg( $requirement = 'enabled' ) {
			...
			/* translators: %1$s: plugin name, %2$s: send email option, %3$s: status */

			return sprintf( __( 'The option "%1$s > %2$s" needs to be %3$s.', 'emails-verification-for-woocommerce' ),
