...
if ( ! class_exists( 'Alg_WC_Email_Verification_Settings_Email' ) ) :
	...
	class Alg_WC_Email_Verification_Settings_Email extends
		...
		function get_settings() {
			...
			$general_opts            = array(
				...
				array(
					...
					'desc'              => sprintf( __( 'Possible values: %1$s, %2$s, %3$s.', 'emails-verification-for-woocommerce' ), __( 'Plain', 'emails-verification-for-woocommerce' ), __( 'WooCommerce', 'emails-verification-for-woocommerce' ), __( 'Smart', 'emails-verification-for-woocommerce' ) ),
			...
			$email_placeholder_opts  = array(
				array(
					...
					'desc'  => sprintf( __( 'These placeholders can be used on the Email Subject or Content options: %s.', 'emails-verification-for-woocommerce' ), '<code>' . implode( '</code>, <code>', $this->get_default_email_placeholders() ) . '</code>' ) . ' ' .
					           ...
					           sprintf( __( 'If you\'re having deliverability problems or emails being sent to Spam, try using the placeholders %1$s and %2$s on the Email Content or subject.', 'emails-verification-for-woocommerce' ), '<code>%site_title%</code>', '<code>%site_url%</code>' ),
			...
			$activation_email_opts   = array(
				...
				array(
					...
					'desc_tip' => __( 'Disable it if you want to append it to the standard WooCommerce "Customer new account" email.', 'emails-verification-for-woocommerce' ) . ' ' .
					              /* translators: %1$s, %2$s placeholder(s) */
					              sprintf( __( 'If the %1$s email is not being sent, try to change the option %2$s.', 'emails-verification-for-woocommerce' ), __( 'New account', 'emails-verification-for-woocommerce' ), '<strong>' . __( 'Email sending trigger', 'emails-verification-for-woocommerce' ) . '</strong>' ),
					'type'     => 'checkbox',
				...
				array(
					...
					/* translators: %1$s, %2$s placeholder(s) */
					'desc_tip' => sprintf( __( 'It\'s necessary to add %1$s to the %2$s email template.', 'emails-verification-for-woocommerce' ), '<code>do_action( "alg_wc_ev_activation_email_content_placeholder", $email->object )</code>', '"Customer new account"' ) . '<br />' .
		...
		function get_frequency_description() {
			...
			return sprintf( __( 'Send email every %1$s %2$s.', 'emails-verification-for-woocommerce' ), (int) $frequency === 1 ? '' : $frequency, $formatted_words[ $unit ] );
		}
