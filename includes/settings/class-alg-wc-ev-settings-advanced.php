...
if ( ! class_exists( 'Alg_WC_Email_Verification_Settings_Advanced' ) ) :
	...
	class Alg_WC_Email_Verification_Settings_Advanced extends
		...
		function get_settings() {
			$advanced_opts = array(
				...
				array(
					...
					'desc_tip' => __( 'WordPress filter used to check user authentication.', 'emails-verification-for-woocommerce' ) . '<br />' .
					              ...
					              sprintf( __( 'Use the %1$s filter to priorize the %2$s message over the %3$s message.', 'emails-verification-for-woocommerce' ), 'authenticate', __( 'verification error', 'emails-verification-for-woocommerce' ), __( 'incorrect password', 'emails-verification-for-woocommerce' ) ),
					'type'     => 'select',
					'class'    => 'chosen_select',
			...
			$prevent_login_after_register_opts = array(
				...
				array(
					...
					/* translators: %1$s, %2$s, %3$s placeholder(s) */
					'desc'    => sprintf( __( 'Note: The %1$s method sets the %2$s filter as %3$s.', 'emails-verification-for-woocommerce' ), '<strong>' . __( 'login filter from WooCommerce', 'emails-verification-for-woocommerce' ) . '</strong>', '<code>woocommerce_registration_auth_new_customer</code>', '<code>false</code>' ),
					'type'    => 'select',
					'class'   => 'chosen_select',
				...
				array(
					...
					'desc_tip'      => sprintf( __( '"%1$s" must be selected for the "%2$s" option above.', 'emails-verification-for-woocommerce' ),
			...
			$delete_opts = array(
				array(
					...
					'desc'  => __( 'Some notes regarding how this tool works:', 'emails-verification-for-woocommerce' ) . '<br />'
					           /* translators: %1$s, %2$s placeholder(s) */
					           . '- ' . sprintf( __( 'Every deleted user  will be registered on %1$s page over the source %2$s.', 'emails-verification-for-woocommerce' ), sprintf( '<a href="%s">' . __( 'WooCommerce logs', 'emails-verification-for-woocommerce' ) . '</a>', admin_url( 'admin.php?page=wc-status&tab=logs' ) ), '<code>alg_wc_ev_delete_users_tool</code>' ) . '<br />'
					           /* translators: %s placeholder(s) */
		...
		function get_delete_users_cron_info() {
			...
			if ( empty( self::$delete_users_cron_output ) ) {
				$output = '';
				if (
					( ! $event_timestamp = wp_next_scheduled( 'alg_wc_ev_delete_unverified_users' ) )            // phpcs:ignore WordPress.Security.NonceVerification.Missing				&& isset( $_POST['alg_wc_ev_delete_users_cron'] )
				) {
					$output = '<br />';
					...
					/* translators: %s placeholder(s) */
					$output .= sprintf( __( 'Next event scheduled to %s', 'emails-verification-for-woocommerce' ), '<strong>' . get_date_from_gmt( gmdate( 'Y-m-d H:i:s', $event_timestamp ), get_option( 'date_format' ) . ' - ' . get_option( 'time_format' ) ) . '</strong>' );
